...
def addUser(user_ip, json_data):
	...
	if user_ip not in devices["ips"]:
		...
		devices["details"].append({
			"Status": "Active",
			"device_ip": user_ip,
			"Devices_name": json_data.get("DeviceName", "Unknown Device"),
			"User_name": json_data.get("Username", "Unknown User"),
			"OS": json_data.get("OS", "Unknown OS")
		})
	else:
		for d in devices["details"]:
			if d["device_ip"]==user_ip:
...
def log_this_one():
	def hooker(func):
		...
		def func_returner(*args, **kwargs):
			...
			print(f'{user_ip} - - [{current_time}] "{method} {current_endpoint} {proto}" ')
			if request.is_json:
				json_data = request.get_json()
				if json_data["RequestType"]=="packet":
					savePacket(json_data["Deviceip"], json_data.get("data", {}))
				else:
